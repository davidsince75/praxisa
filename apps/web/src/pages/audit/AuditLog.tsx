...
function eventVariant(
  eventType: string,
): "default" | "destructive" | "completed" | "pending" | "in_progress" | "rejected" {
...
export function AuditLogPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get<AuditEventsResponse>(`/audit/events?${params.toString()}`),
  });

  const totalPages = data !== undefined ? (data.pagination.count < PAGE_SIZE ? (page + 1) : page + 2) : 0;

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-dark">Journal d'audit</h1>
        <p className="text-meta text-sm mt-1">Trace immuable de toutes les actions système</p>
      </div>
        ...
        <CardContent className="p-0">
          ...
          {(data?.events.length ?? 0) > 0 && (
            <div className="grid grid-cols-12 px-6 py-2 border-b border-rule bg-cream">
              <span className="col-span-3 text-xs font-bold uppercase tracking-wider text-meta">Date</span>
              <span className="col-span-3 text-xs font-bold uppercase tracking-wider text-meta">Type</span>
              <span className="col-span-3 text-xs font-bold uppercase tracking-wider text-meta">Entité</span>
              <span className="col-span-3 text-xs font-bold uppercase tracking-wider text-meta">Acteur</span>
            </div>
          )}

          <ul className="divide-y divide-rule">
            {data?.events.map((ev) => (
              <li key={ev.id} className="grid grid-cols-12 px-6 py-3 items-center hover:bg-cream/50 transition-colors">
                <span className="col-span-3 text-xs text-meta">{formatDate(ev.eventAt)}</span>
                ...
                <span className="col-span-3 text-xs text-meta font-mono">
                  {ev.actorUserId !== null ? ev.actorUserId.slice(0, 8) + "…" : "system"}
                </span>
              </li>
            ))}
          </ul>
