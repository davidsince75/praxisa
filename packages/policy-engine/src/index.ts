...
const HARD_BLOCKS: Partial<Record<PolicyAction, string>> = {
  'grade:publish': 'HARD_BLOCK_AI_GRADE_PUBLISH — AI actor may never publish grades',
};
...
export function evaluate(request: PolicyRequest): PolicyResult {
  ...
  if (request.actor.type === 'ai') {
    ...
    if (hardBlock) {
      return { allowed: false, reasonCode: hardBlock, policyVersion: POLICY_VERSION, decisionId };
    }
  }

  // Workers and system actors may not impersonate users
  if (request.action === 'admin:impersonate' && request.actor.type !== 'user') {
    return { allowed: false, reasonCode: 'NON_USER_IMPERSONATION_DENIED', policyVersion: POLICY_VERSION, decisionId };
  }

  // DSR transitions require admin or gdpr_officer role
  if (request.action === 'dsr:transition' || request.action === 'dsr:evidence_export') {
    if (!request.actor.roles.some(r => ['admin', 'gdpr_officer'].includes(r))) {
      return { allowed: false, reasonCode: 'INSUFFICIENT_ROLE_FOR_DSR', policyVersion: POLICY_VERSION, decisionId };
    }
  }
  ...
  if (request.action === 'pii:bulk_export') {
    ...
    if (!hasRole || !hasReason) {
      return { allowed: false, reasonCode: 'BULK_EXPORT_REQUIRES_ADMIN_AND_REASON', policyVersion: POLICY_VERSION, decisionId };
    }
  }
  ...
  if (request.action === 'ai:execute_tier3') {
    if (!request.actor.roles.includes('instructor')) {
      return { allowed: false, reasonCode: 'TIER3_AI_REQUIRES_INSTRUCTOR_ROLE', policyVersion: POLICY_VERSION, decisionId };
    }
  }
  ...
  if (request.action === 'migration:execute') {
    if (!request.actor.roles.includes('migration_lead')) {
      return { allowed: false, reasonCode: 'MIGRATION_REQUIRES_MIGRATION_LEAD_ROLE', policyVersion: POLICY_VERSION, decisionId };
    }
  }

  return { allowed: true, reasonCode: 'ALLOWED', policyVersion: POLICY_VERSION, decisionId };
}
